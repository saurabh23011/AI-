...
def process_image(image):
    ...
    identified_objects = identify_objects(extracted_objects)
    
    # Step 4: Text/Data Extraction from Objects
    extracted_text = extract_text(extracted_objects)
    
    # Step 5: Summarize Object Attributes
    summarized_attributes = summarize_attributes(identified_objects, extracted_text)
    
    # Step 6: Data Mapping
    mapped_data = map_data(extracted_objects, identified_objects, extracted_text, summarized_attributes)
    ...
    if st.button("Process Image"):
        ...
        for obj in mapped_data:
            with st.expander(f"Object {obj['id']} - {obj['category']}"):
                ...
                st.write(f"Extracted Text: {obj['extracted_text']}")
